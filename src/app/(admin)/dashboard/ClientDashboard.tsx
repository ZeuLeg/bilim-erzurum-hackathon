...
export default function ClientDashboard({ workOrders, pendingReportsCount, totalWorkOrders }: ClientDashboardProps) {
  ...
  const handleRunThirtyDayRiskAnalysis = async () => {
    await sendMessage({
      text: 'Sistemdeki iş emirlerini inceleyerek önümüzdeki 30 gün içinde çakışma riski doğabilecek lokasyonları ve müdürlükleri tespit et. Her risk için müdürlük adını, tarih aralığını ve önerilen koordinasyon aksiyonunu maddeler halinde Türkçe olarak listele.',
    });
  };
  ...
  return (
    <main className="min-h-screen bg-slate-50">
      ...
      <div className="mx-auto max-w-7xl px-6 py-8">
        {!impactError ? (
          <section className="rounded-3xl border border-emerald-200 bg-gradient-to-r from-emerald-50 to-sky-50 p-5 shadow-sm">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <h2 className="text-lg font-semibold text-slate-900">AI Çakışma Önleme Etkisi</h2>
              <span className="text-xs text-slate-500 bg-white rounded-full px-2 py-1 border">Bu Sezon</span>
            </div>
            <div className="grid grid-cols-1 gap-4 mt-4 pt-4 border-t border-emerald-100 sm:grid-cols-3">
              <div>
                {impactLoading ? (
                  <div className="rounded-2xl bg-slate-200 animate-pulse h-20 w-full" />
                ) : (
                  <>
                    <p className="text-2xl font-bold text-slate-900">₺{conflictImpact ? conflictImpact.wastedBudgetTRY.toLocaleString('tr-TR') : '0'}</p>
                    <p className="mt-2 text-xs text-slate-500">önlenebilir kayıp</p>
                  </>
                )}
              </div>
              <div>
                {impactLoading ? (
                  ...
                  <>
                    <p className="text-2xl font-bold text-slate-900">{conflictImpact ? conflictImpact.co2KgSaved.toLocaleString('tr-TR') : '0'} kg</p>
                    <p className="mt-2 text-xs text-slate-500">kg CO₂ azaltma</p>
                  </>
                )}
              </div>
              <div>
                {impactLoading ? (
                  ...
                  <>
                    <p className="text-2xl font-bold text-slate-900">{conflictImpact ? conflictImpact.roadMetersSaved.toLocaleString('tr-TR') : '0'} m</p>
                    <p className="mt-2 text-xs text-slate-500">metre yol kurtarıldı</p>
                  </>
                )}
              </div>
            </div>
          ...
          <div className="flex-1 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <button
                  type="button"
                  onClick={handleRunThirtyDayRiskAnalysis}
                  disabled={isLoading}
                ...
                workOrders.map((order) => (
                  <article key={order.id} className="rounded-3xl border border-slate-200 bg-slate-50 p-5">
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      <span className="rounded-full bg-orange-100 px-2 py-0.5 text-xs font-semibold text-orange-800">SDG 9 · Altyapı</span>
                      <span className="rounded-full bg-yellow-100 px-2 py-0.5 text-xs font-semibold text-yellow-800">SDG 11 · Sürd. Şehir</span>
                      <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-semibold text-emerald-800">SDG 13 · İklim</span>
                    </div>
                  </article>
                ))
              ) : (
